...
# 4. 6개 탭 구성 (스캐너 신규 탑재 ⭐)
tab_scanner, tab_analysis, tab_team_stats, tab_h2h, tab_injuries, tab_input = st.tabs([
    "📡 라운드 100경기 자동 스캐너",
    "📊 9개사 동일 배당 분석", 
    "📈 팀별 세부내용 평균계산기",
    "⚔️ 홈 vs 원정 맞대결 종합분석",
...
# =========================================================
# TAB 0: 📡 라운드 100경기 자동 스캐너 & 레이더 (신규 탑재 ⭐)
# =========================================================
with tab_scanner:
    st.subheader("📡 라운드 100경기 배당 자동 스캐너 & 4대 레이더")
    st.caption("구글 시트의 `라운드스캔` 탭에 적어둔 이번 라운드 경기들을 1초 만에 분석하여, 블로그에 글 쓰기 가장 좋은 추천 경기들을 카테고리별로 자동 선별합니다.")

    df_scan_raw = load_sheet_data(SCANNER_SHEET_NAME)
    df_h2h_all_db = load_sheet_data(STATS_SHEET_NAME)

    if df_scan_raw.empty:
        st.warning(f"⚠️ 구글 시트의 `{SCANNER_SHEET_NAME}` 탭에 데이터가 비어 있습니다. 이번 라운드 경기들의 팀명과 배당을 입력해 주세요.")
    else:
                ...
                with st.container(border=True):
                    ...
                    with c_head3:
                        ...
                        if st.button(f"👉 이 경기 즉시 분석/도표 생성", key=f"btn_scan_{i}", type="primary", use_container_width=True):
                            ...
                            st.success(f"🎉 [{item['home']} vs {item['away']}] 경기 데이터가 2번/4번 탭으로 전송되었습니다! 상단의 [📊 9개사 동일 배당 분석] 또는 [⚔️ 맞대결 종합분석] 탭을 확인하세요.")
...
with tab_analysis:
    ...
    # 스캐너에서 전달받은 경기 데이터가 있을 경우 자동 채우기
    auto_home = "리버풀"
    auto_away = "본머스"
    auto_league = "PL"
    auto_odds = {}
...
with tab_h2h:
    ...
    # 스캐너에서 전달받은 팀명이 리스트에 있을 경우 자동 선택
    if st.session_state.selected_scan_match:
        sm = st.session_state.selected_scan_match
